...
def parse_metar_full(raw_line):
    ...
    for i, p in enumerate(parts):
        ...
        if 'SM' in p:
            ...
            continue
        if '/' in p and not p.startswith('/') and not p.endswith('/') and 'SM' not in p:
            clean = p.replace('M','').replace('/','').replace('-','')
            if clean.isdigit():
                t_parts = p.split('/'); 
                if len(t_parts) == 2: temp, dew = t_parts[0], t_parts[1]
            continue
        if p.startswith('A') and len(p) == 5 and p[1:].isdigit(): qnh = p; continue
        if not stop_wx:
...
ax[3].plot(df['Timestamp'], df['MWS_Wind_Spd'], 'g-', label='MWS')
ax[3].plot(df['Timestamp'], df['CYYR_Wind_Spd'], 'r-', label='CYYR')
ax[3].set_ylabel('kt'); ax[3].grid(True, alpha=0.3); ax[3].set_title('Windstärke')
